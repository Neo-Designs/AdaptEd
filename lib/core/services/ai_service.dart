...
class AIService {
  ...
  Future<String> _callGeminiAPI(String userContent,
    ...
    if (_geminiApiKey.isEmpty) {
      AppLogger.error('API Key Missing',
          tag: 'AIService', error: 'GEMINI_API_KEY not found in .env');
    ...
    try {
      ...
      return await _retryWithBackoff(
          () async {
            ...
            final response =
                await model.generateContent([Content.text(userContent)]);
  ...
  Future<List<Map<String, dynamic>>> generateShortAnswerQuiz(
    ...
    final prompt =
        "Generate exactly 10 short-answers based on the following text."
