...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  void _startTypingAnimation(int messageIndex, String fullText) {
    ...
    _typingTimer = Timer.periodic(delay, (timer) {
      ...
      final int batchSize = safeRunes.length > 2000 ? 5 : 1;
      // 2. Iterate using the safe runes length
      if (charIndex < safeRunes.length) {
  ...
  void _showDifficultyDialog() {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          backgroundColor: theme.cardColor,
          title: Text('Select Quiz Difficulty', style: theme.titleStyle),
          content: Column(
  ...
  void _navigateToQuiz({String difficulty = 'MEDIUM'}) {
    // Navigate straight to the Assessment screen passing both the PDF text AND difficulty!
  ...
  Widget build(BuildContext context) {
    ...
    // 1. Detect if the software keyboard has popped up!
                ...
                Expanded(
                  child: Padding(
                    padding: EdgeInsets.only(
                        top: 24, 
                        left: theme.interactivePadding, 
                        right: theme.interactivePadding),
                    child: _buildChatArea(theme),
                  ),
                ),
                Visibility(
                  visible: !isKeyboardOpen || _reviewFocusNode.hasFocus,
                  child: Padding(
                    // Changed bottom padding to 0 so it hugs the bottom bar
                    padding: EdgeInsets.fromLTRB(theme.interactivePadding, 8,
                        theme.interactivePadding, 10),
    ...
    return GestureDetector(
      onTap: () {
        if (title == 'Upload a PDF') {
          ...
          // For 'Ask anything', just quietly focus the keyboard.
  ...
  Widget _buildBottomInputBar(DynamicTheme theme) {
    // 1. Detect keyboard state natively inside this widget block
    final bool isKeyboardOpen = MediaQuery.of(context).viewInsets.bottom > 0;
        ...
        itemBuilder: (context, index) {
          ...
          return GestureDetector(
            onTap: () {
              ...
              // 2. Routing Logic
              if (chip['label'] == '🧪 Quiz me') {
                // If Quiz, pop up the difficulty dialog!
                _showDifficultyDialog();
              } else {
                // Otherwise, execute the background prompt silently!
                _executeQuickActionSilently(chip['prompt']!, theme);
              }
            },
            child: Container(
...
class _TypewriterMarkdownState extends State<TypewriterMarkdown> {
  ...
  void initState() {
    super.initState();
    _safeRunes = widget.data.runes.toList();
    _startTyping();
  }
  ...
  void _startTyping() {
    _timer = Timer.periodic(const Duration(milliseconds: 8), (timer) {
      if (!mounted || !_isTyping || _currentIndex >= _safeRunes.length) {
        timer.cancel();
